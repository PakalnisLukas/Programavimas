...
int generate_random() //generuojame pazymius
{
	return rand() % 10 + 1;
}
...

{
	...
	std::vector<duomenys> teigiami;
	std::vector<duomenys> neigiami;
	int neig = 0;
	int teig = 0;
	...
	for (int i = 0; i < kiek; i++) {
		...
		if (studentai.at(i).GP < paz) {
			teigiami.push_back(studentai.at(i));
			neig++;
		}
	}
	for (int j = 0; j < kiek; j++) {
		...
		if (studentai.at(j).GP >= paz) {
			teigiami.push_back(studentai.at(j));
			teig++;
		}
	}
	auto end = std::chrono::high_resolution_clock::now();
	...
	for (int i = 0; i < kiek; i++) {
		...
		float paz = 5.00;
		if (studentai.at(i).GP < paz) {
			neig_failas << studentai.at(i).Vard << std::setw(20) << studentai.at(i).Pav << std::setw(18) << studentai.at(i).GP << std::endl;
		}
